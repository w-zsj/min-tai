    ...
    <view class="main-wrapper">
      ...
      <view class="section">
              ...
              <view class="section-detail-right flex-col-btwn">
                <view>
                  <view class="section-detail-amount">
                    ...
                    <!-- 普通商品价格 -->
                    <view v-if="item.productPrice > 0">
                      {{ item.useCoin > 0 ? "+" : "" }}฿{{ item.productPrice }}
                    </view>
                  </view>
        ...
        <view class="section-item section-num">
          <view class="section-item-title">订单总价</view>
          <view class="section-item-right flex-aic">
            <text v-if="totalAmount > 0">฿{{ totalAmount }}</text>
          </view>
        </view>
        <view class="section-item section-num">
          <view class="section-item-title">配送费</view>
          <view class="section-item-right flex-aic">
            <text v-if="totalAmount > 0">฿{{ freightAmount }}</text>
          </view>
        </view>
        <view class="section-item section-deliver">
          <view class="section-item-title">嗨币</view>
      ...
      <view class="bottom line" :class="{ ipx: Inipx }">
        <view class="bottom-btn bottom-btn-left" v-if="orderStatus === 3"
          @click.stop="handleBtnOrder('confirmReceiveOrder')">确认收货
        </view>
        <view class="bottom-btn bottom-btn-left" v-if="orderStatus === 0"
          @click.stop="handleBtnOrder('cancelUserOrder')">取消订单</view>
        <view class="bottom-btn bottom-btn-left" v-if="orderStatus == 5" @click.stop="handleBtnOrder('again')">再次购买
        </view>
        <view class="bottom-btn bottom-btn-left" v-if="orderStatus == 4" @click.stop="handleBtnOrder('goShopping')">去逛逛
        </view>
        <view class="bottom-btn" v-if="orderStatus === 0 || orderStatus === 2" @click.stop="handleBtnOrder('pay')">立即支付
          <text v-if="countDown && orderStatus === 0"> {{ date.m }}:{{ date.s }} </text>
        </view>
      </view>
    </view>
  </view>
