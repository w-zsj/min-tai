<template>
	<view class="header-container" :style="{marginTop:marginTop+'rpx'}">
		<swiper class="detail-swiper" :indicator-dots="indicatorDots" :indicator-active-color="activeColor" :autoplay="autoplay"
			:interval="interval" :circular="circular" :duration="duration" @animationfinish="changeGoodsSwip"
			:current="goodsIndex" @change="swiperChange">
			<swiper-item v-if="detail.video">
				<view class="video-swiper-item">
				<video
				  class="video-container"
				  id="demoVideo"
				  :poster="detail.video+'?x-oss-process=video/snapshot,t_0,f_jpg'"
				...
				>
					<image v-if="isShowPlayBtn" src="@/static/images/video.png" class="detail-play-btn" @click="parsePlay"></image>
				</video>
				</view>
			</swiper-item>
		...
		<view :class="{'swiper-dot-container': true,'video-swiper-dot': detail.video}" v-if="imageList.length || detail.video ">
		...
		methods: {
				...
				if (detail.detail.source == 'touch') {
					...
					if (detail.detail.current == 0) {
						...
						if (swiperError >= 2) {
							...
							this.setData({ goodsIndex: this.preIndex }); //，重置current为正确索引
							this.setData({ swiperError: 0 });
						}
					} else {
						//正常轮播时，记录正确页码索引
						this.setData({ preIndex: detail.detail.current }); //将开关重置为0
						this.setData({ swiperError: 0 });
					}
				}
			...
			onVideoFullScreen(e) {
				if(!this.fullScreenStatus){
					...
					this.setData({marginTop:0});
				}else{
					this.videoContext.exitFullScreen();
					this.fullScreenStatus=false;
					this.setData({marginTop:104});
				}
			},  
			// 播放
			parsePlay: function() {
				this.videoContext.play();
				this.setData({isShowPlayBtn: false,controls: true});
			},
			// 轮播图切换
